...
public struct AdMobNativeAdHostView: UIViewRepresentable {
    ...
    // AdMobNativeAdHostUIView 를 그대로 노출하지 않고 UIView 로 감춘다
    public func makeUIView(context: Context) -> UIView {
        AdMobNativeAdHostUIView(contentView: makeContentView())
    }

    public func updateUIView(_ uiView: UIView, context: Context) {
        // 광고가 갱신되면 레이아웃에 반영, nil 이면 기본 콘텐츠로 복원
        (uiView as? AdMobNativeAdHostUIView)?.configure(with: ad)
    }
}
